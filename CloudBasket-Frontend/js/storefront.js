...
async function loadProducts() {
    ...
    try {
        ...
        if (CUSTOMER_ID && CUSTOMER_ID !== 'cust-001') {
            try {
                const wishRes = await apiFetch(`https://rua1bnesw8.execute-api.ap-southeast-1.amazonaws.com/api/v1/wishlist/${CUSTOMER_ID}`);
                if (wishRes.ok) {
...
async function loadMarketingBanner() {
    try {
        ...
        try {
            const couponRes = await fetch('https://rua1bnesw8.execute-api.ap-southeast-1.amazonaws.com/api/v1/marketing/coupons');
        ...
        try {
            const festRes = await fetch('https://rua1bnesw8.execute-api.ap-southeast-1.amazonaws.com/api/v1/marketing/festival-sales/active');
...
window.toggleWishlist = async function(event, id, btnElement) {
    ...
    try {
        ...
        if (isAdded) {
            const response = await apiFetch(`https://rua1bnesw8.execute-api.ap-southeast-1.amazonaws.com/api/v1/wishlist/${CUSTOMER_ID}/${id}`, {
                method: 'DELETE'
            });
            if (response.ok) {
